...
module tb_single_cycle_mips();
    ...
    always @ (posedge done) begin
        ...
        if (lw_sw_test_pass == `LW_SW_TEST_LEN) begin
            $display("LW/SW PASSED %d test cases", lw_sw_test_pass);
        end
        else begin
            $display("LW/SW FAILED %d test cases", `LW_SW_TEST_LEN - lw_sw_test_pass);
        end

        // ARITHMETIC TEST

        if (add_test_pass == `ADD_TEST_LEN) begin
            $display("ADD PASSED %d test cases", add_test_pass);
        end
        else begin
            $display("ADD FAILED %d test cases", `ADD_TEST_LEN - add_test_pass);
        end

        if (sub_test_pass == `SUB_TEST_LEN) begin
            $display("SUB PASSED %d test cases", sub_test_pass);
        end
        else begin
            $display("SUB FAILED %d test cases", `SUB_TEST_LEN - sub_test_pass);
        end

        if (slt_test_pass == `SLT_TEST_LEN) begin
            $display("SLT PASSED %d test cases", slt_test_pass);
        end
        else begin
            $display("SLT FAILED %d test cases", `SLT_TEST_LEN - slt_test_pass);
        end

        if (addi_test_pass == `ADDI_TEST_LEN) begin
            $display("ADDI PASSED %d test cases", addi_test_pass);
        end
        else begin
            $display("ADDI FAILED %d test cases", `ADDI_TEST_LEN - addi_test_pass);
        end

        if (slti_test_pass == `SLTI_TEST_LEN) begin
            $display("SLTI PASSED %d test cases", slti_test_pass);
        end
        else begin
            $display("SLTI FAILED %d test cases", `SLTI_TEST_LEN - slti_test_pass);
        end

        // BRANCH TEST

        if (beq_test_pass == `BEQ_TEST_LEN) begin
            $display("BEQ PASSED %d test cases", beq_test_pass);
        end
        else begin
            $display("BEQ FAILED %d test cases", `BEQ_TEST_LEN - beq_test_pass);
        end

        if (bne_test_pass == `BNE_TEST_LEN) begin
            $display("BNE PASSED %d test cases", bne_test_pass);
        end
        else begin
            $display("BNE FAILED %d test cases", `BNE_TEST_LEN - bne_test_pass);
        end

        // JUMP TEST

        if (j_test_pass == `J_TEST_LEN) begin
            $display("J PASSED %d test cases", j_test_pass);
        end
        else begin
            $display("J FAILED %d test cases", `J_TEST_LEN - j_test_pass);
        end

        if (jal_test_pass == `JAL_TEST_LEN) begin
            $display("JAL PASSED %d test cases", jal_test_pass);
        end
        else begin
            $display("JAL FAILED %d test cases", `JAL_TEST_LEN - jal_test_pass);
        end

        if (ra_test_pass == `RA_TEST_LEN) begin
            $display("$RA PASSED %d test cases", ra_test_pass);
        end
        else begin
            $display("$RA FAILED %d test cases", `RA_TEST_LEN - ra_test_pass);
        end

        if (jr_test_pass == `JR_TEST_LEN) begin
            $display("JR PASSED %d test cases", jr_test_pass);
        end
        else begin
            $display("JR FAILED %d test cases", `JR_TEST_LEN - jr_test_pass);
        end

        if (sll_test_pass == `SLL_TEST_LEN) begin
            $display("SLL PASSED %d test cases", sll_test_pass);
        end
        else begin
            $display("SLL FAILED %d test cases", `SLL_TEST_LEN - sll_test_pass);
        end

        if (srl_test_pass == `SRL_TEST_LEN) begin
            $display("SRL PASSED %d test cases", srl_test_pass);
        end
        else begin
            $display("SRL FAILED %d test cases", `SRL_TEST_LEN - srl_test_pass);
        end
